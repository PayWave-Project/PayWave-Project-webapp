...
import { useGetBankList, useSendMoney, useVerifyBank } from "@/api/wallet";
...
const SendMoneyModal = () => {
  ...
  const { mutateAsync: sendMoney, isLoading: sendingMoney } = useSendMoney();

  const bankLists: Bank[] = data?.data?.data || [];

  const [activeSection, setActiveSection] = useState<
    "bankTransfer" | "paywave" | null
  >(null);
  ...
  const handleNextStep = (section: "bankTransfer" | "paywave") => {
  ...
  return (
    <div className="space-y-4">
      {activeSection !== "paywave" && (
        <div>
          <div>
            {activeSection === "bankTransfer" && step === 1 && (
              <div className="w-full flex flex-col gap-4">
                ...
                <div className="flex justify-between">
                  ...
                  <button
                    ...
                    onClick={() => handleNextStep("bankTransfer")}
                  >
                    Next
                  </button>
                </div>
            ...
            {activeSection === "bankTransfer" && step === 2 && (
              <>
                {sendingMoney ? (
      ...
      {/* {activeSection !== "bankTransfer" && (
        <div
          className="flex items-center gap-4 p-4 border border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md"
          onClick={() => handleNextStep("paywave")}
        >
          <Send className="w-6 h-6" />
          <div>
            {activeSection === "paywave" && step === 1 && (
              <>
                <h2 className="text-xl font-bold">Step 1: Enter Paywave ID</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Please enter the recipient's Paywave ID.
                </p>
              </>
            )}
            {activeSection === "paywave" && step === 2 && (
              <>
                <h2 className="text-xl font-bold">Step 2: Confirm Transfer</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">
            ...
            {activeSection !== "paywave" && (
              <>
                <h2 className="text-xl font-bold">Paywave to Paywave</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Send money to a Paywave user
                </p>
              </>
            )}
          </div>
